...
const EMAIL_FROM = process.env.EMAIL_FROM || 'no-reply@nextgen-chat.com';
const SITE_URL = process.env.SITE_URL || process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000';

if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !SENDGRID_API_KEY) {
    console.error('Missing required environment variables for password reset.');
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        const sgResp = await fetch('https://api.sendgrid.com/v3/mail/send', {
            ...
            body: JSON.stringify({
                personalizations: [{ to: [{ email }], subject: 'Reset your password' }],
              ...
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="text-align:center; margin: 30px 0;">
                  <a href="${linkToSend}" style="display:inline-block;padding:12px 24px;background:#8b5cf6;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;">
                    Reset Password
                  </a>
                </p>
